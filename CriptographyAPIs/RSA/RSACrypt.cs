    ...
    public class RSACrypt
    {
        public static ObjectResult Crypt(RSAmodel data)
        {
            try
            {
                ...
                BigInteger p = BigInteger.Parse(data.P);
                BigInteger q = BigInteger.Parse(data.Q);

                if (p == q || !IsPrime(p) || !IsPrime(q))
                {
                    throw new ArgumentException("Mistyped input");
                }
                ...
                if (n < 255)
                {
                    throw new ArgumentException("Error, try to use bigger prime numbers for p and q,for example 17 and 19");
                }
        ...
        private static bool IsPrime(BigInteger number) //utilizzando l'algoritmo aks
        {
